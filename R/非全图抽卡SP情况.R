...
for(i in 1:N){
  ...
  while(1){
    ...
    if(up<3)
      ...
      p1 <- 0.0125
    if(runif(1, max = 1,min = 0) < p1){
      up <- up + 1
      if(runif(1, max = 1,min = 0) < p) break
    }
  }
  l[i] <- n
  ssr[i] <- up
...
ggsave('非全图SPcdf.png', g, path = "./plot" )
ggsave('非全图SP频率分布.png',f, path = "./plot"  )
ggsave('非全图SP箱型图.png',h, path = "./plot"  )
ggsave('非全图SP黑屏数箱型图.png',h1, path = "./plot"  )
